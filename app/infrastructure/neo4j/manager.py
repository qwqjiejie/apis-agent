...
class Neo4jManager:
    ...
    async def initialize(self, uri: str = "", user: str = "", password: str = ""):
        ...
        try:
            ...
            self._driver = AsyncGraphDatabase.driver(
                uri,
                auth=(user, password),
                connection_timeout=get_settings().external_connect_timeout_seconds,
            )
            # 连通性验证
    ...
    async def health_check(self) -> HealthCheckResult:
        if not self._driver:
            return HealthCheckResult("Neo4j", False, "not configured")
        try:
            await retry_async(
                self._driver.verify_connectivity,
                attempts=get_settings().external_retry_attempts,
            )
            return HealthCheckResult("Neo4j", True)
        except Exception as exc:
            return HealthCheckResult("Neo4j", False, str(exc))
