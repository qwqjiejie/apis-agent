...
class VectorStore:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self._client = MilvusClient(
                uri=uri,
                token=token,
                timeout=get_settings().external_connect_timeout_seconds,
            )
    ...
    def health_check(self) -> HealthCheckResult:
        if not get_settings().milvus_host:
            return HealthCheckResult("Milvus", False, "not configured")
        if self.connect():
            return HealthCheckResult("Milvus", True)
        return HealthCheckResult("Milvus", False, self._connect_error or "unavailable")
