...
async def file_list(req: FileListRequest, request: Request):
    files, total = await run_in_threadpool(
...
async def file_upload(
    ...
    user_id = get_current_user_id(request)
    if conversationId and not (
        await inspect_session_access(request, conversationId)
    ).allowed:
...
async def file_delete(req: FileDetailRequest, request: Request):
    ok_deleted = await run_in_threadpool(
        file_service.delete,
        req.fileId,
        user_id=get_current_user_id(request),
    )
