...
| 编号 | 功能 | 描述 |
|------|------|------|
| F-01 | 智能问答 (WebSearch) | 基于 ReAct 模式的联网搜索 + 推理，实时获取互联网信息后回答用户问题；调用 Tavily MCP 搜索引擎 |
| F-02 | 文件问答 (File RAG) | 上传 PDF/DOC/DOCX/TXT/图片文件后进行语义问答；支持全文直读和 RAG 向量检索两种模式 |
| F-03 | PPT 生成 (PPT Builder) | 基于模板驱动的 PPT 自动生成，含意图识别(CREATE/MODIFY/RESUME)、Schema 生成、大纲生成、内容填充、AI 配图、多格式渲染(Python-pptx / PptxGenJS) |
| F-04 | 深度研究 (Deep Research) | Plan-Execute-Critique 多轮自主研究：需求澄清 -> 研究主题生成 -> 计划生成 -> 多轮执行 -> 自我批判 -> 综合报告 |
| F-05 | 技能助手 (Skills) | 通用型 ReAct Agent，集成搜索/文件/Skills/文件系统/Bash/Grep 等所有工具，LLM 自动判断使用哪个工具 |

### 2. 会话管理功能

| 编号 | 功能 | 描述 |
|------|------|------|
| F-06 | 会话 CRUD | 分页查询会话列表、查询会话详情、删除会话（级联删除关联的 file_info 和 ppt_inst） |
| F-07 | 对话历史持久化 | 用户问题和 AI 回答（含 thinking/工具调用/参考来源/推荐问题）存储到 MySQL |
| F-08 | 跨会话记忆恢复 | 通过 ChatMemory 从数据库加载历史消息，支持上下文延续 |

### 3. 文件管理功能

| 编号 | 功能 | 描述 |
|------|------|------|
| F-09 | 文件上传 | 支持 PDF/DOC/DOCX/TXT/PNG/JPG，上传到 MinIO 对象存储 |
| F-10 | 文件解析 | 文本文件解析提取内容（PDFBox + POI）；大文件自动切分向量化（500 字符块 + 50 字符重叠） |
| F-11 | 图片识别 | 调用多模态模型 (qwen3-vl-plus) 识别图片内容 |
| F-12 | 文件 RAG 检索 | 查询压缩重写 -> 多查询扩展 -> PgVector 语义检索 -> 去重合并 |
| F-13 | 文件删除/查询 | 删除文件和 MinIO 对象、检查文件存在、获取文件信息 |

### 4. Agent 控制功能

| 编号 | 功能 | 描述 |
|------|------|------|
| F-14 | 任务停止 | 用户可随时停止正在执行的 Agent，支持当前实例直停 + Redis Pub/Sub 跨实例广播 |
| F-15 | 任务并发控制 | 基于 Redis 分布式锁的会话级任务去重，同一会话不允许同时执行多个 Agent |
| F-16 | 优雅中断 | Reactor Disposable 机制确保子任务链完全取消 |

### 5. 流式输出功能

| 编号 | 功能 | 描述 |
|------|------|------|
| F-17 | SSE 流式响应 | 所有 Agent 统一使用 SSE (text/event-stream) 输出 |
| F-18 | 思考过程展示 | `<think>` 标签解析，思考内容（thinking）和最终答案（text）分离输出 |
| F-19 | 工具调用追踪 | tool_start / tool_end 事件实时展示工具调用状态 |
| F-20 | 参考来源 | reference 类型消息，展示搜索结果的 URL/标题 |
| F-21 | 推荐问题 | 每轮对话结束自动生成 3 个推荐问题 |

### 6. 上下文管理功能

| 编号 | 功能 | 描述 |
|------|------|------|
| F-22 | 上下文压缩 (Layer 1) | 每轮自动压缩旧工具结果和长参数为占位符 |
| F-23 | 上下文压缩 (Layer 2) | Token 超过阈值后 LLM 摘要压缩所有旧消息 |
| F-24 | Token 估算 | TokenEstimator 估算消息列表 token 数 |

### 7. 工具集功能

| 编号 | 功能 | 描述 |
|------|------|------|
| F-25 | Tavily 搜索 | 通过 MCP 协议接入 Tavily 搜索引擎，自动解析搜索结果 |
| F-26 | Bash 工具 | 持久化 Shell 会话，执行系统命令，支持超时和输出限制 |
| F-27 | 文件系统工具 | read_file / write_file / edit_file / list_files / glob_files |
| F-28 | Grep 工具 | 正则表达式内容搜索，优先使用 ripgrep，回退到 Java 原生 |
| F-29 | Skills 工具 | 从本地目录或 classpath 加载 SKILL.md，注册为可调用的工具 |
