...
async def _do_rebuild(app: FastAPI):
    from src.apis_agent.agent.agent_factory import (
        create_triage_agent, create_executor_agent,
        _build_subagents_from_specialists, _build_external_tools,
    )
    from src.apis_agent.prompt.triage_prompt import build_triage_prompt
...
async def lifespan(app: FastAPI):
    ...
    from src.apis_agent.stores.pg_store import pg_store_manager
    ...
    from src.apis_agent.agent.agent_factory import create_triage_agent, create_executor_agent
    from src.apis_agent.prompt.triage_prompt import build_triage_prompt
    from src.apis_agent.prompt.executor_prompt import build_executor_prompt
    from src.apis_agent.tool import TOOL_REGISTRY
    ...
    # ── 9. 事件总线 ─────────────────────────────────
    from src.apis_agent.harness.event_bus import EventBus
    app.state.event_bus = EventBus()

    # ── 10. 死信扫描 ────────────────────────────────
    from src.apis_agent.harness.dead_letter import dead_letter_queue
    ...
    # ── 12. SubAgent 热加载 ──────────────────────────
    from src.apis_agent.harness.subagent_hot_reloader import SubAgentHotReloader
